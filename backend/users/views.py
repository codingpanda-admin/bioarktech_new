...
from users.serializers import AddressSerializer, UserSerializer, CustomerShippingAddressSerializer
...
@api_view(['POST', 'PUT'])
def upsert_billing_address(request):
    """Create the signed-in user's billing address, or update their sole address."""
    if not request.user.is_authenticated:
        return Response({'detail': 'User is not authenticated.'}, status=401)
    ...
    zipcode = str(data.get('zipcode', data.get('postal_code', ''))).strip()

    if not all([address_line_1, city, state, zipcode]):
        return Response({'detail': 'Address line 1, city, state, and ZIP/postal code are required.'}, status=400)

    with transaction.atomic():
        # Lock the user row so simultaneous first-time saves cannot attach two records.
        user = User.objects.select_for_update().get(id=request.user.id)
        # Load the nullable relation separately. PostgreSQL cannot apply FOR UPDATE
        # to the nullable side of the outer join generated by select_related().
        billing_address = (
            Address.objects.filter(id=user.billing_address_id).first()
            if user.billing_address_id
            else None
        )
        created = billing_address is None

        if created:
            billing_address = Address()

        billing_address.address_line_1 = address_line_1
        billing_address.address_line_2 = str(data.get('address_line_2', '') or '').strip()
        billing_address.apt_suite = str(data.get('apt_suite', '') or '').strip()
        billing_address.city = city
        billing_address.state = state
        billing_address.zipcode = zipcode
        billing_address.country = str(data.get('country', 'US') or 'US').strip()
