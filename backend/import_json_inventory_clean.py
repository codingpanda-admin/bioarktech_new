...
with transaction.atomic():
    ...
    ManualFile.objects.all().delete()
    ProductsUnion.objects.all().delete()
...
def import_model_data(model_class, model_key, pk_name, mapper_func):
    ...
    print(f"Importing {len(items)} items for {model_key}...")
    success_count = 0
    for item in items:
        pk = item['pk']
...
print(f"Importing/Updating {len(products_to_import)} products in the main catalog...")
p_count = 0
for item in products_to_import:
    ...
    try:
        ...
        if p_obj:
            ...
            # Create letting database generate the identity PK
