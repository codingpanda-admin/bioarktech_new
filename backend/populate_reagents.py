...
def main():
    ...
    # 1. Locate reagents.json
    possible_paths = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'reagents.json'),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), '../reagents.json'),
        '/app/reagents.json',
        'reagents.json',
    ]
    ...
    if not json_path:
        print("Error: Could not locate reagents.json")
        sys.exit(1)
    ...
    reagents_list = data.get("reagents", [])
    overrides = data.get("overrides", {})
    details = data.get("details", {})
    groups_config = data.get("groupsConfig", {})
    
    # 2. Extract reagent category IDs
    reagent_cat_ids = [cat['id'] for cat in categories_data]
    print(f"Target category IDs to clean and rebuild: {reagent_cat_ids}")
    
    # 3. Clear existing reagents and their images/relations from database
    # Get all products belonging to these categories
    products_to_delete = Product.objects.filter(category_external_id__in=reagent_cat_ids)
    print(f"Deleting {products_to_delete.count()} existing products from these categories...")
    
    # Delete their images relations first
    ProductImage.objects.filter(product__in=products_to_delete).delete()
    products_to_delete.delete()
    
    # Delete the categories themselves to rebuild them
    ProductCategory.objects.filter(external_id__in=reagent_cat_ids).delete()
    
    # 4. Create Reagent Categories
    created_categories = {}
    for cat in categories_data:
        cat_id = cat['id']
        cat_name = cat['name']
        cat_order = cat.get('order', 1)
        cat_type = cat.get('productType', 'reagent' if 'reagent' in cat_id or 'reagent' in cat_name.lower() else 'consumable')
        if 'consumable' in cat_id or 'consumable' in cat_name.lower():
            cat_type = 'consumable'
    ...
    # 5. Merge and Parse Reagents
    parsed_products = {}
    
    # - Add from reagents list
    for p in reagents_list:
        pid = p["id"]
        parsed_products[pid] = {
            'external_id': pid,
            'catalog_number': pid,
    ...
    print(f"Successfully populated {len(parsed_products)} reagents/consumables.")
