...
def load_reagents_from_json(apps, schema_editor):
    ...
    possible_paths = [
        os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'reagents.json'),
    ...
    if not json_path:
        raise FileNotFoundError("Could not locate reagents.json in migrations loader.")
    ...
    # Reagent category IDs
    reagent_cat_ids = [cat['id'] for cat in categories_data]
    
    # Delete existing products under these categories
...
def unload_reagents_from_json(apps, schema_editor):
    ProductCategory = apps.get_model('products', 'ProductCategory')
    Product = apps.get_model('products', 'Product')
    ProductImage = apps.get_model('products', 'ProductImage')
    
    # Just clear products in these category external IDs
    reagent_cat_ids = [
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(load_reagents_from_json, unload_reagents_from_json),
    ]
